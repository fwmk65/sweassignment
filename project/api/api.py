...
def search(start, end):
    ...
    Database query 
    """
    # Parsing URL parameters so that they match the database entry
    # Room names are lower case and use spaces instead of %20
    while "%20" in start:
        for i, char in enumerate(start[:-3]):
            if start[i:i + 3] == "%20":
                start = start[:i] + ' ' + start[i+3:]
                break

    while "%20" in end:
        for i, char in enumerate(end[:-3]):
            if end[i:i + 3] == "%20":
                end = end[:i] + ' ' + end[i+3:]
                break
    ...
    start = db.execute(f'SELECT ID FROM Rooms WHERE Name = "{start}"')
    end = db.execute(f'SELECT ID FROM Rooms WHERE Name = "{end}"')
    for row in start:
        start = row[0] - 1
    for row in end:
        end = row[0] - 1
    ...
    return {"path": generate_path(start, end, prev)}
...
def audio_request():
    ...
    if places == None:
        ...
        return redirect("search/<start>/<end>")
